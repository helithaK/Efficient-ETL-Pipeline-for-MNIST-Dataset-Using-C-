...
void data_handler::split_data()
{
    ...
    std::uniform_int_distribution<> dis(0, data_array->size() - 1); // Range for indices

    //Training data
    int count = 0;
    while (count < train_size)
    {
        int rand_index = dis(gen); // 0 & data_array->size() - 1
...
void data_handler::count_classes()
{
    ...
    num_classes = count;
    printf("Successfully Extracted %d unique classes./n", num_classes);
}
